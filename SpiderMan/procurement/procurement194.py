...
class Procurement1(ProcurementBaseSpider):
    name = "Procurement_194"
    base_link = ''
    hospital_name = '无锡市第三人民医院'

    def start_requests(self):
        # 初始页
        urls = []
        for i in range(27):
            list_url = "http://www.wuxihospital.com//wszb/zbxx/{}.htm".format(i+1)
            urls.append(list_url)
        urls.append("http://www.wuxihospital.com//wszb/zbxx.htm")
        print(urls)
        params = {
            # "hospital": "1010",
            # "category": "32",
        ...
        self.hospital_url = 'http://www.wuxihospital.com/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//body").extract()[0]
        href_list = re.findall('/info[^.]+?.htm', context)

        for each in href_list:
            article_url = self.hospital_url + each
            yield scrapy.FormRequest(url=article_url, callback=self.articleparse,
                                     method='GET')

    def articleparse(self, response: HtmlResponse):
        title = response.xpath("//div[@class='article-tt text-center']/text()").extract()[0]
        ori_url = response.url
        release_date = response.xpath("//div[@class='article-b text-center']/i/text()").extract()[0]
        release_date = release_date[3:15]
        mainbody = response.xpath("//div[@id='vsb_content']").extract()[0]
