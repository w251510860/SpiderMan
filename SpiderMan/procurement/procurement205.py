...
class Procurement1(ProcurementBaseSpider):
    name = "Procurement_205"
    base_link = ''
    hospital_name = '宜兴市人民医院'
    def start_requests(self):
        # 初始页
        urls = []
        for i in range(27):
            list_url = 'http://www.yxph.com/default.php?mod=article&fid=2300&s44714398_start={}'.format(i*10)
        ...
        self.hospital_url = 'http://www.yxph.com/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//span[@class='link_13']/a/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath('//font[@style="FONT-SIZE: 20px"]/text()').extract()[0]
        ori_url = response.url
        release_date = response.xpath('//p/font[@class="link_14"]/text()').extract()[0]
        # release_date = release_date.split("：")[3][0:10]
        mainbody = response.xpath('//div[@id="ohid_s22891322"]').extract()[0]
