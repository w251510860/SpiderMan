...
class Procurement33(ProcurementBaseSpider):
    name = "Procurement_33"
    base_link = ''
    ...
    def start_requests(self):
        ...
        urls = []
        for i in range(72):
            list_url = 'http://www.szwy.cn/showclass-10-{}.html'.format(i + 1)
        ...
        self.hospital_url = 'http://www.szwy.cn/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//table/tr/td/table/tr/td/a[@target='_blank']/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath('//div[@class="aTitle"]/text()').extract()[0]
        ori_url = response.url
        release_date = response.xpath('//div[@class="aDate"]/text()').extract()[0]
        release_date = release_date.split("：")[1][0:10]
        mainbody = response.xpath('//div[@class="aContent"]').extract()[0]
