...
class Procurement1(ProcurementBaseSpider):
    name = "Procurement_204"
    base_link = ''
    hospital_name = '江阴市中医院'
    def start_requests(self):
        # 初始页
        urls = []
        for i in range(5):
            list_url = 'https://www.jyzyy.com.cn/f/menu/1333644439995957248?pageNo={}'.format(i + 1)
        ...
        self.hospital_url = 'https://www.jyzyy.com.cn/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//div[@class='newslist']/ul/li/a/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath("//span[@style='font-size:16px;']/strong/text()").extract()[0]
        ori_url = response.url
        release_date = response.xpath("//p[@class='MsoNormal'][last()]/span/text()").extract()[0]
        mainbody = response.xpath("//div[@class='r-c']").extract()[0]
