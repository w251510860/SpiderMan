...
class Procurement1(ProcurementBaseSpider):
    ...
    def start_requests(self):
        ...
        for i in range(3):
            if i == 0:
                list_url = 'http://www.pumcderm.net/list/59.html'
            else:
                list_url = 'http://www.pumcderm.net/list/59_{}.html'.format(i + 1)
            urls.append(list_url)
        print(urls)
        params = {

        }
        self.hospital_url = 'http://www.pumcderm.net/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//ul[@class='news_lb1 md40']/li/a/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath('//h2[@class="sbt md15"]/text()').extract()[0]
        ori_url = response.url
        release_date = response.xpath('//div[@class="ssj md35 zt_9d"]/text()').extract()[0]
        release_date = release_date.split("：")[1][0:10]
        mainbody = response.xpath('//div[@class="ny"]').extract()[0]
