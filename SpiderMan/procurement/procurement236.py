...
class Procurement1(ProcurementBaseSpider):
    name = "Procurement_236"
    base_link = ''
    hospital_name = '宜兴市和桥医院'
    ...
    def start_requests(self):
        ...
        for i in range(8):
            if i == 0:
                list_url = 'http://www.yxshqyy.com/yiyuandongtai/'
            else:
                list_url = 'http://www.yxshqyy.com/yiyuandongtai/list_{}.html'.format(i + 1)
        ...
        self.hospital_url = 'http://www.yxshqyy.com/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//div[@class='new-list wow fadeInUp']/ul/li/a/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath("//h1[@class='wow fadeInUp']/text()").extract()[0]
        ori_url = response.url
        release_date = response.xpath('//time/text()').extract()[0]
        # release_date = release_date.split("：")[3][0:10]
        mainbody = response.xpath('//section[@class="container page clearfix"]').extract()[0]
