...
class Procurement9(ProcurementBaseSpider):
    name = "procurement10"
    base_link = ''
    hospital_name = '苏州科技城医院'
    ...
    def start_requests(self):
        ...
        for i in range(30):
            if i == 0:
                list_url = "https://www.szkjcyy.com/channels/74.html"
            else:
                list_url = 'https://www.szkjcyy.com/channels/74_{}.html'.format(i + 1)
        ...
        self.hospital_url = 'https://www.szkjcyy.com/'
        # 遍历、翻页
    ...
    def parse(self, response: HtmlResponse):
        # 解析列表页
        # 测试请求是否成功
        context = response.xpath("//div[@class='news_list']/ul/li/a/@href")
    ...
    def articleparse(self, response: HtmlResponse):
        title = response.xpath('//div[@class="dis_title"]/h1/text()').extract()[0]
        ori_url = response.url
        release_date = response.xpath('//div[@class="dis_title"]/p/text()').extract()[0]
        release_date = release_date[0:10]
        mainbody = response.xpath('//div[@class="Content"]').extract()[0]
